...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///diseaseinfo.db'
db=SQLAlchemy(app)
    ...
    #   @marshal_with(token_resource_field)
      def post (self):
        ...
        if check_password_hash(user.password, password):
            usertoken = jwt.encode({'id' : user.id, 'exp' : datetime.datetime.utcnow() + datetime.timedelta(minutes=30)}, app.config['SECRET_KEY'])
            decodedtoken = jwt.decode(usertoken,app.config['SECRET_KEY'], algorithms=["HS256"])
